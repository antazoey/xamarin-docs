...
Markup similar to this is the basis of the **VSM on View** page in the sample program. The page includes three `Entry` views, but only the second one has the VSM markup attached to it:
...
The **VSM with Setter TargetName** page in the sample shows how to set state on multiple elements, from a single visual state group. The XAML file consists of a `StackLayout` containing a `Label` element, an `Entry`, and a `Button`:
...
The **VSM Validation** page in the sample shows how to use the Visual State Manager in connection with input validation. The XAML file consists of a `StackLayout` containing two `Label` elements, an `Entry`, and a `Button`:
