...
As an example, we are going to create a sample app called **MonkeyBrowser**. This app will present four tabs, each with a different URL open in a web browser view. The user will be able to continue any tab on a different iOS device running the app.
...
An example of using Handoff in a Xamarin.iOS app is the **MonkeyBrowser** sample app. The app has four tabs that the user can use to browse the web, each with a given activity type: Weather, Favorite, Coffee Break and Work.
